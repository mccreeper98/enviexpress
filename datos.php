...
if(isset($_POST['reason'])){
    ...
    $consultaCp = mysqli_query($connect, "SELECT disponibilidad.idDisponibilidad, disponibilidad.extendida, paqueterias.imagen, paqueterias.nombre, paqueterias.idPaqueteria FROM disponibilidad INNER JOIN paqueterias ON paqueterias.idPaqueteria = disponibilidad.idPaqueteria WHERE disponibilidad.idZona = '$cpd'");
    ...
    if($num_disponible == 0){
        ...
        while($disponibilidad = mysqli_fetch_array($consultaCp)){
            $aux++;
            $response .= '<a onclick="selectPaq('.$aux.', '.$num_disponible.', '.$disponibilidad["idPaqueteria"].', \''.$disponibilidad["nombre"].'\', \''.$disponibilidad["extendida"].'\')"><div class="paq" id="paq'.$aux.'"><img src="img/paqueterias/'.$disponibilidad['imagen'].'" class="paqueteria"></div></a>';
        }
    }
